...
- 很多算法都是有模板的，要有意识去背模板，当然也需要理解。我们刷的算法并没有那么高大上，不需要发挥大家的创造力，大家只要理解并背诵即可

## 知识点介绍
搜索分为 DFS、BFS，这两个算法都是有模板的，要有意识去背模板。DFS 最自然的是用递归实现，DFS 改为迭代实现的意义并不大，BFS 天生就是迭代实现的。在实现上，DFS 会借助递归实现，BFS 会借助队列实现。
